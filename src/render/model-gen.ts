...
import type { TemplateFile, TemplateVars } from '../types.js';

const MODEL_MARKER = '_model_';

export async function renderPerModel(
  modelTemplates: TemplateFile[],
  targetDir: string,
  vars: TemplateVars,
): Promise<void> {
  ...
  for (const file of modelTemplates) {
    ...
    for (const model of vars.models) {
      const modelVars: TemplateVars = { ...vars, model };
